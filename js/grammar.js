...
const Grammar = {
    ...
    _storageKey: 'el_grammar_progress',
    ...
    init() {
        ...
        if (this.currentIndex > 0) {
            setTimeout(() => {
                const el = document.querySelector('.grammar-resume-notice');
                if (el) el.remove();
                const container = document.getElementById('grammarContainer');
                if (container) {
                    const notice = document.createElement('div');
                    notice.className = 'grammar-resume-notice';
                    notice.innerHTML = `<i class="fas fa-history"></i> 已恢复到第 ${this.currentIndex + 1} 题 <a href="javascript:Grammar.retry()" style="margin-left:12px;color:#6366f1;text-decoration:underline;">重新开始</a>`;
