/* ============================================
   作文练习模块 — 通义千问 VL Max OCR
   阿里云百炼平台（qwen-vl-max），国内直连，专为视觉识别优化
...
const Writing = {
    ...
    renderPrompt() {
        ...
        if (isTeacher) {
            keySectionHtml = ''
                ...
                + ' → 模型广场 → 搜索「qwen-vl-max」→ 选「通义千问VL」→ 获取Key，国内直连免翻墙'
                + '    </p>'
                + '  </details>'
                + '</div>';
        } else if (!hasKey) {
            keySectionHtml = ''
                ...
                + '    <i class="fas fa-info-circle"></i> 手写识别功能尚未配置，请联系老师开启。'
                + '  </p>'
                + '</div>';
        } else {
            keySectionHtml = ''
                ...
                + '    <i class="fas fa-check-circle"></i> AI 手写识别已就绪，上传图片即可使用'
                + '  </p>'
                + '</div>';
        }
        ...
        container.innerHTML = ''
            ...
            + '    </span>'
            + '    ' + (hasKey ? '<span style="padding:6px 14px;background:rgba(16,185,129,0.1);color:#10b981;border-radius:20px;font-size:0.85rem;font-weight:600;"><i class="fas fa-robot"></i> 通义AI 识别</span>' : '')
            + '  </div>'
            + '</div>'
            + keySectionHtml
    ...
    async saveApiKey() {
        ...
        if (saved) {
            App.toast('API Key 已保存到云端，所有学生均可使用', 'success');
        } else {
    ...
    renderScore(score) {
        ...
        resultDiv.innerHTML = [
            ...
            '  <h3><i class="fas fa-chart-bar" style="color:var(--primary);"></i> 评分结果</h3>',
